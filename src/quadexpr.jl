...
function setobjective(m::Model, sense::Symbol, a::QuadExpr)
    ...
    MOI.setobjective!(m.instance, moisense, MOI.ScalarQuadraticFunction(a))
    nothing
end
...
function objectivefunction(m::Model, ::Type{QuadExpr})
    f = MOI.getattribute(m.instance, MOI.ObjectiveFunction())::MOI.ScalarQuadraticFunction
    return QuadExpr(m, f)
end
...
function constraintobject(cref::ConstraintRef{Model}, ::Type{QuadExpr}, ::Type{SetType}) where {SetType <: MOI.AbstractScalarSet}
    m = cref.m
    f = MOI.getattribute(m.instance, MOI.ConstraintFunction(), cref.instanceref)::MOI.ScalarQuadraticFunction
    s = MOI.getattribute(m.instance, MOI.ConstraintSet(), cref.instanceref)::SetType
    return QuadExprConstraint(QuadExpr(m, f), s)
end

# function Base.copy(c::QuadExprConstraint, new_model::Model)
